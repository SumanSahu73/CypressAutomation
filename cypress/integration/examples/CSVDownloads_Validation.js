...
describe('JWT Session', () => {
  ...
  //LoginAPI is custom command which we hava made in command.js
     ...
     // cy.log(Cypress.config("fileServerFolder"))
      ...
      //cy.readFile(Cypress.config("fileServerFolder")+"\\cypress\\downloads\\order-invoice_suman0703sahu.csv")
    
      //cy.readFile("cypress\\downloads\\order-invoice_suman0703sahu.csv")
      .then(async(text)=>
      {
       ...
       //extracting the all csv content as a text but it will not care about row column index no./ position
